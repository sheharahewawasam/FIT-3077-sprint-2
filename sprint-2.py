    ...
    for row in range(tiles_per_row):
        for col in range(tiles_per_row):
            if row == 0 or row == tiles_per_row - 1 or col == 0 or col == tiles_per_row - 1:
                ...
                original_x = (col * (rhombus_width + spacing_width)) - (tiles_per_row * (rhombus_width + spacing_width)) // 4
                original_y = (row * (rhombus_height // 2 + spacing_height)) - (tiles_per_row * (rhombus_height // 2 + spacing_height)) // 4

                # Rotated coordinates
                center_x = int(start_x + original_x - original_y)
                center_y = int(start_y + original_x + original_y)
